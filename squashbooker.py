...
def book_squash(target_datetime):
    ...
    print()
    print ("Navigating to booking sheets page")
    ...
    print()
    print('Navigating to Booking sheet for', date_str)
...
def cancel_tennis_courts_to_avoid_fees():
    #++++++++++++++++++++++++++++++++++++++++++++++++
    cancel_date=datetime.today() + timedelta(days=1)
    #++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    for booking in bookings_table:
        ...
        if int(cancel_date.strftime('%d')) < 10:
            cancel_date_to_use = cancel_date.strftime('%A %d %B %Y').split()
            cancel_date_to_use[1] = cancel_date_to_use[1].replace('0','')
        else:
            cancel_date_to_use = cancel_date.strftime('%A %d %B %Y')
        if 'select opponent(s)' in str(booking) and cancel_date.strftime('%A %d %B %Y') in str(booking):
...
#set date as today + timedelta = 7
#set time accourding to what day it is

desired_date = '13 December 2020'   #Long date format to avoid confusion
desired_time = '13:00' #input 08:00, 09:00, 13:00 etc
court = 'tennis'  # either "squash" or "tennis"
court_type = 'acrylic' #clay or acrylic
...
court_date = datetime.strptime(f'{desired_date}', "%d %B %Y").strftime("%Y-%m-%d")  #str
court_datetime = datetime.strptime(court_date, '%Y-%m-%d').date()    #datetime.date(2020, 12, 10) : datetime.date
#print()
#print('Booking',court,'court for',desired_time,'on', desired_date)
if court_datetime >= today:
    target_date = datetime.strptime(desired_date, '%d %B %Y')
    target_time = datetime.strptime(desired_time, '%H:%M').time() 
    target_datetime = datetime.combine(target_date, target_time)        #datetime.datetime(2020, 12, 12, 18, 30)
